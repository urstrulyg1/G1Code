const instructionLike =
  /ignore\s+(all\s+)?previous instructions|system message|developer message|run\s+(rm|sudo)|exfiltrat|send\s+.*secret|api\s*key|password/i;
...
export function wrapUntrusted(
  ...
  const warning = instructionLike.test(content)
    ...
    : "";
  return {
    content: `[UNTRUSTED ${source}]${warning}\n${content}\n[/UNTRUSTED ${source}]`,
    source,
    trusted: false,
  };
}
