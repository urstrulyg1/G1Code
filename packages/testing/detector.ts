...
export type ProjectType =
  "npm" | "maven" | "gradle" | "go" | "cargo" | "python" | "make";
export type Project = {
  type: ProjectType;
  root: string;
  testCommand: string;
...
export async function detectProject(root: string): Promise<Project | null> {
  ...
  if (packageFile) {
    try {
      ...
      const script = ["test", "test:unit", "test:integration"].find(
  ...
  if (
    ...
    (await exists("setup.cfg"))
  )
    return {
      type: "python",
      root,
      testCommand: "python -m pytest",
      executable: process.platform === "win32" ? "python.exe" : "python",
      testArgs: ["-m", "pytest"],
    };
