...
export function targetedExecutable(
  ...
  if (project.type === "npm") args.push("--", first);
    ...
    args.splice(1, 0, `-Dtest=${path.basename(first).replace(/\.[^.]+$/, "")}`);
