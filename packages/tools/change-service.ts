...
export class ChangeService {
  ...
  async proposeChange(
    ...
    const change = this.store.addChange({
      ...
      proposedHash: contentHash(proposedContent),
      originalContent,
      proposedContent,
      appliedContent: null,
  ...
  getChange(id: string) {
    return this.store.getChange(id);
  }
  listPendingChanges(sessionId?: string) {
    return this.store.pendingChanges(sessionId);
  }
  ...
  authorizeChange(id: string, sessionId: string) {
    ...
    if (
      ...
      session.workspaceId !== this.workspace
    )
  ...
  approveChange(id: string) {
    ...
    if (change.status !== "PENDING")
      throw new Error(`Cannot approve ${change.status} change`);
  ...
  rejectChange(id: string) {
    const change = this.requireChange(id);
    if (change.status !== "PENDING")
      throw new Error(`Cannot reject ${change.status} change`);
  ...
  async applyChange(id: string) {
    ...
    if (change.status !== "APPROVED")
      throw new Error(`Change must be approved before applying`);
  ...
  async applyBatch(sessionId: string, ids: string[]) {
    ...
    if (changes.some((change) => change.status !== "APPROVED"))
      throw new Error("Every change must be approved before batch apply");
    const paths = new Set<string>();
    for (const change of changes) {
      if (paths.has(change.path))
        throw new Error(`Duplicate path in change batch: ${change.path}`);
    ...
    const batch = this.store.createChangeBatch(
      {
        id: batchId,
        sessionId,
    ...
    const tempRoot = await fs.mkdtemp(
      path.join(this.workspace, ".g1code-batch-"),
    );
    ...
    try {
      ...
      for (const change of changes) {
        ...
        if (contentHash(prepared) !== change.proposedHash)
          throw new Error(
            `Temporary content verification failed: ${change.path}`,
          );
      ...
      for (const change of changes) {
        if (
          this.store.transitionChangeStatus(
            change.id,
            "APPROVED",
            "APPLYING",
          ) !== 1
          ...
          throw new Error(
            `Change state changed during batch preparation: ${change.path}`,
          );
      }
      ...
      try {
        ...
        return {
          batch: this.store.changeBatch(batchId),
          changes: changes.map((change) => this.requireChange(change.id)),
        };
      } catch (error) {
        if (
          error instanceof SimulatedCrashError &&
          (error.point === "AFTER_FILE_REPLACE" ||
            error.point === "BEFORE_DB_COMMIT")
        ) {
          throw error;
        }
        this.store.updateChangeBatch(
          batchId,
          "ROLLING_BACK",
        ...
        for (const change of changes) {
          ...
          if (current?.status === "APPLYING")
            this.store.updateChangeStatus(change.id, "APPROVED");
        }
        FailureInjector.maybeCrash("AFTER_ROLLBACK");
        this.store.updateChangeBatch(
          batchId,
          rollbackFailed ? "PARTIAL_FAILURE" : "ROLLED_BACK",
          error instanceof Error ? error.message : String(error),
        );
        if (rollbackFailed)
          throw new Error(
            `Change batch partially failed; review batch ${batchId}`,
          );
        throw new Error(
          `Change batch rolled back after failure; batch ${batchId}`,
        );
      }
    } catch (error) {
      if (
  ...
  async recoverActiveBatches() {
    for (const batch of this.store.activeChangeBatches()) {
      ...
      for (const item of items) {
        const current = await fs
          .readFile(await safeRealPath(this.workspace, item.path), "utf8")
          .catch(() => null);
        allOriginal &&=
          current !== null && contentHash(current) === item.originalHash;
        allProposed &&=
          current !== null && contentHash(current) === item.proposedHash;
      }
      if (allOriginal) {
        this.store.updateChangeBatch(
          batch.id,
          "ROLLED_BACK",
          "Recovered before filesystem replacement",
        );
        for (const item of items) {
          this.store.updateChangeBatchItem(
            batch.id,
            item.changeId,
            "ROLLED_BACK",
          );
          ...
          if (change?.status === "APPLYING")
            this.store.updateChangeStatus(item.changeId, "APPROVED");
        }
      } else if (allProposed) {
        this.store.updateChangeBatch(
          batch.id,
          "APPLIED",
          "Recovered after filesystem replacement",
        );
        for (const item of items) {
          this.store.updateChangeBatchItem(batch.id, item.changeId, "APPLIED");
          const change = this.store.getChange(item.changeId);
          if (change?.status === "APPROVED" || change?.status === "APPLYING") {
            this.store.updateAppliedContent(
              item.changeId,
        ...
        this.store.updateChangeBatch(
          batch.id,
          "PARTIAL_FAILURE",
          "Filesystem differs from both batch journal states",
        );
        for (const item of items)
          this.store.updateChangeBatchItem(
            batch.id,
  ...
  async revertChange(id: string) {
    ...
    if (this.store.transitionChangeStatus(id, "APPLIED", "APPLYING") !== 1)
      throw new Error(
        "Change is already being reverted or is no longer applied",
      );
    const result = await revertAppliedChange(
      this.workspace,
      change.path,
      change.proposedHash,
      change.originalContent,
  ...
  private requireChange(id: string) {
