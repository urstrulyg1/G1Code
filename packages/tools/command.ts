...
function spawnSpec(
  ...
  const safeEnvironment = Object.fromEntries(
    Object.entries(command.env ?? process.env).filter(
      ([key]) =>
        !/(KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL|COOKIE|AUTH)/i.test(key),
    ),
  );
  ...
  const limit = 1_000_000;
  const collect = (target: "stdout" | "stderr") => (value: string) => {
    if (target === "stdout") stdoutText += value;
    else stderrText += value;
    if (stdoutText.length + stderrText.length > limit) {
      stdoutText = stdoutText.slice(0, 500_000);
      stderrText = stderrText.slice(0, 500_000);
      truncated = true;
      cancel();
    }
  };
