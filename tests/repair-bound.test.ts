...
test("agent bounds repeated failing test repairs", async () => {
  ...
  const provider: AIProvider = {
    ...
    streamChat: async function* () {
      calls += 1;
      yield { toolCalls: [{ id: `test-${calls}`, name: "run_tests", arguments: { paths: [] } }] };
    },
  };
  ...
  tools.register({
    ...
    execute: async () => ({ content: "tests failed", isError: true, exitCode: 1 }),
  });
  const states: string[] = [];
  await new AgentRuntime(provider, tools, "/workspace", (event) => { if (event.state) states.push(event.state); }, async () => true, undefined, "session").run("test", "agent");
