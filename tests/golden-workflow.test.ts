...
test("golden workflow proposes, approves, tests, repairs, and retests a real fixture", async () => {
  ...
  await cp(
    path.join(process.cwd(), "tests/fixtures/agent-project"),
    workspace,
    { recursive: true },
  );
    ...
    streamChat: async function* (_request: ChatRequest) {
      ...
      if (providerStep === 1 || providerStep === 3) {
        ...
        yield {
          toolCalls: [
            {
              id: `test-${providerStep}`,
              name: "run_tests",
              arguments: { paths: ["src/calculator.js"] },
            },
          ],
        };
      } else {
  ...
  registry.register({
    ...
    execute: async (value, context) => {
      ...
      const current = await readFile(
        path.join(context.workspace, input.path),
        "utf8",
      );
      const change = await changes.proposeChange(
        sessionId,
        input.path,
      ...
      new Promise((resolve) => {
        ...
        void (async () => {
          ...
          resolve({
            ...
            message: applied.status === "APPLIED" ? "approved" : "conflict",
          });
  ...
  const run = runtime.run(
    "Fix the calculator implementation and make sure all tests pass.",
    "agent",
  );
  await Promise.race([
    run,
    new Promise((_, reject) =>
      setTimeout(
        () =>
          reject(
            new Error(
              `golden workflow timed out at provider step ${providerStep}`,
            ),
          ),
        10_000,
      ),
