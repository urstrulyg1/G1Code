...
import {
  scanRepository,
  extractSymbols,
} from "../packages/indexing/repository";
...
test("indexing extracts symbols and context suppresses unchanged files", async () => {
  ...
  assert.equal(
    context.assemble([
      { kind: "file", path: "src/sample.ts", content: "same", priority: 2 },
    ]).length,
    1,
  );
  assert.equal(
  ...
  assert.equal(
    context.assemble([
      { kind: "file", path: "src/sample.ts", content: "changed", priority: 2 },
    ]).length,
    1,
  );
});
...
test("context compaction preserves task state and failures", () => {
  ...
  const summary = context.compact({
    objective: "Fix auth",
    plan: ["Inspect service", "Run tests"],
    discoveries: ["Maven project"],
    changes: ["AuthService.java"],
    tests: ["AuthServiceTest passed"],
    failures: ["Expiration case failed"],
    nextAction: "Diagnose expiration",
  });
