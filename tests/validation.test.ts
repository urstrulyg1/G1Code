...
import { requireAction, requireBoundedString, requireObject } from "../packages/security/validation";
...
test("IPC validators reject malformed and oversized values", () => {
  ...
  assert.throws(() => requireBoundedString("x".repeat(11), "id", 10), /Invalid id/);
  assert.throws(() => requireObject([], "request"), /Invalid request/);
  assert.throws(() => requireAction("delete", ["approve", "reject"]), /Invalid action/);
