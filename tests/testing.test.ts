...
test("detects npm projects and ranks related tests", async () => {
  ...
  const candidates = await discoverTests(root, ["src/calculator.js"], project ?? undefined);
  assert.equal(candidates[0]?.path, "tests/calculator.test.js");
  assert.equal(targetedCommand(project!, candidates), "npm run test -- tests/calculator.test.js");
});
