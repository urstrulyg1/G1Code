...
async function runE2ESuite() {
  ...
  const testWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "g1code-e2e-ws-"));
  ...
  try {
    ...
    console.log("[1/6] Launching Electron desktop window in validation mode...");
    ...
    console.log("      Electron desktop application launched and mounted successfully (SMOKE_LOAD_SUCCESS).");
    ...
    assert.equal(serverOk, true, "Backend API server failed to respond on http://127.0.0.1:3131");
    ...
    const chooseRes = await fetch("http://127.0.0.1:3131/api/workspace/choose", {
      method: "POST",
    ...
    assert.ok(listRes.some((e: any) => e.name === "main.ts" && e.kind === "file"));
    ...
    await fetch("http://127.0.0.1:3131/api/file/write", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ filePath: "main.ts", contents: 'console.log("updated e2e");\n' }),
    });
    ...
    const termRes = await fetch("http://127.0.0.1:3131/api/terminal/run", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ command: "echo E2E_TERMINAL_SUCCESS", cwd: testWorkspace }),
    }).then((r) => r.json());
    ...
    console.log("[6/6] Testing diagnostics aggregation and Git baseline endpoints...");
    const problemsRes = await fetch(`http://127.0.0.1:3131/api/problems?workspace=${encodeURIComponent(testWorkspace)}`).then((r) => r.json());
    assert.ok(Array.isArray(problemsRes.problems));

    const gitRes = await fetch(`http://127.0.0.1:3131/api/git/status?workspace=${encodeURIComponent(testWorkspace)}`).then((r) => r.json());
