...
test("Provider Resilience: Unknown tool call is reported back gracefully without crashing agent", async () => {
  const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "g1code-provider-res-"));
  try {
    ...
    const runtime = new AgentRuntime(
      ...
      async () => true,
      { maxIterations: 5, maxToolCalls: 10, maxExecutionTime: 5000, maxRepairAttempts: 2 },
    );

    await runtime.run("do something", "agent");
    const completedEvent = events.find((e) => e.state === "COMPLETED");
    assert.ok(completedEvent, "Agent successfully completed after recovering from unknown tool call");
  } finally {
...
test("Provider Resilience: Malformed tool call arguments are safely bounded", async () => {
  const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "g1code-provider-res-2-"));
  try {
    const registry = new ToolRegistry();
    workspaceTools().forEach((tool) => registry.register(tool));
    ...
    const errorEvent = events.find((e) => e.type === "error" && e.toolName === "read_file");
    assert.ok(errorEvent, "Malformed tool arguments produced safe error response");
  } finally {
    await fs.rm(workspace, { recursive: true, force: true }).catch(() => undefined);
  }
});
