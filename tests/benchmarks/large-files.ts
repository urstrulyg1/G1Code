...
async function runLargeFilesBenchmark() {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "g1code-bench-files-"),
  );
  ...
  try {
    ...
    for (const size of sizesMB) {
      ...
      console.log(
        `      Created file: ${filePath} (${(stats.size / 1024 / 1024).toFixed(2)} MB, ${lineCount} lines)`,
      );

      // 2. Benchmark chunked line-based pagination
      console.log(
        `[2/3] Benchmarking line-based pagination on ${size}MB file...`,
      );
      ...
      console.log(
        `      Paging slice (lines 1000-1500) completed in ${pagingDuration}ms`,
      );
      ...
      console.log(
        `[3/3] Benchmarking diff generation on section of ${size}MB file...`,
      );
      const originalSlice = content.slice(0, 500_000);
      const modifiedSlice = originalSlice.replace(
        /Line 00000010/g,
        "Line MODIFIED_10",
      );
      ...
      console.log(
        `      Generated diff (${diff.length} bytes) in ${diffDuration}ms`,
      );
